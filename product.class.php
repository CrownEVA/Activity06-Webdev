...
class Product {
    ...
    public function showAll($keyword='', $category='') {
        // SQL query to fetch the stock levels and availability
        $sql = "SELECT p.name as product_name, 
                       SUM(CASE WHEN s.status = 'In' THEN s.quantity ELSE 0 END) AS total_incoming,
                       SUM(CASE WHEN s.status = 'Out' THEN s.quantity ELSE 0 END) AS total_outgoing,
                       (SUM(CASE WHEN s.status = 'In' THEN s.quantity ELSE 0 END) - 
                        SUM(CASE WHEN s.status = 'Out' THEN s.quantity ELSE 0 END)) AS current_stock,
                       c.name as category_name, p.code, p.price, p.availability, p.id
                FROM product p
                JOIN stocks s ON p.id = s.product_id
                JOIN category c ON p.category_id = c.id
                WHERE (p.code LIKE CONCAT('%', :keyword, '%') OR p.name LIKE CONCAT('%', :keyword, '%'))
